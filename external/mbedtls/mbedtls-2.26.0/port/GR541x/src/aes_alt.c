...
{
    ll_clk_disable_force_off_secu();
    ll_clk_disable_wfi_off_secu();
    ll_clk_disable_force_off_aes();
}

static void aes_clock_disable(void)
{
    ll_clk_enable_force_off_aes();

    if ( ll_clk_get_force_off_secu() == LL_CLK_SECU_HCLK_FRC_OFF )
    {
        ll_clk_enable_force_off_secu();
    }

    if ( ll_clk_get_wfi_off_secu() == LL_CLK_SECU_HCLK_WFI_OFF )
    {
        ll_clk_enable_wfi_off_secu();
    }
}
                           ...
                           unsigned char output[16] )
{
    ...
    /* Set key mode */
    ll_aes_set_key_mode(AES, LL_AES_KEYMODE_NORMAL);
    ...
    AES->DATA_IN0 = p_cryp_input_buffer[0];
    AES->DATA_IN1 = p_cryp_input_buffer[1];
    AES->DATA_IN2 = p_cryp_input_buffer[2];
    AES->DATA_IN3 = p_cryp_input_buffer[3];
    ...
    p_cryp_output_buffer[0] = AES->DATA_OUT0;
    p_cryp_output_buffer[1] = AES->DATA_OUT1;
    p_cryp_output_buffer[2] = AES->DATA_OUT2;
    p_cryp_output_buffer[3] = AES->DATA_OUT3;
